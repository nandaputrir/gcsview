...
// square
        ...
        private void BUT_quickman_Click(object sender, EventArgs e)
        {
            try
            {
                ((Button)sender).Enabled = false;
        ...
        private void BUT_quickcruise_Click(object sender, EventArgs e)
        {
            try
            {
                ((Button)sender).Enabled = false;
        ...
        private void BUT_quickfbwa_Click(object sender, EventArgs e)
        {
            try
            {
                ((Button)sender).Enabled = false;
        ...
        {
            myhud.bgimage = (System.Drawing.Image) sender;
        }
        ...
        string txt;
        Send send2 = new Send();
        private void  aReceived(object sender, SerialDataReceivedEventArgs e)
        {
            txt += serialPort1.ReadExisting().ToString();
            send2.TextBoxValue = txt;
        }
        ...
        void FinalFrame_NewFrame(object sender, NewFrameEventArgs evenArgs)
        {
            Bitmap image = (Bitmap)evenArgs.Frame.Clone();
            Bitmap image1 = (Bitmap)evenArgs.Frame.Clone();
            //pictureBox1.Image = image;
            //pictureBox1.Image = (Bitmap)evenArgs.Frame.Clone();

            // create filter
            EuclideanColorFiltering filter = new EuclideanColorFiltering();
            // set center colol and radius
            filter.CenterColor = new RGB(Color.FromArgb(239, 109, 77));
            filter.Radius = 100;
        ...
        private void tmCountDown_Tick(object sender, EventArgs e)
        {
           // time++;
            //if (time == 2)
            {
               // pictureBox1.Image.Save("E:/FW/GUI/coba"+tambah+".jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
               // tambah++;
            }
        }
        
        private int count, loop, time, coba, tambah;
        string det = "OBJECT DETECTED";
        public void nesnebul(Bitmap image)
        {
            tambah++;
            if ( tambah == 2 || tambah == 5 || tambah == 30 || tambah == 10 || tambah == 20 || tambah == 35)
            {
                try
                {
                    //pictureBox1.Image.Save("E:/FW/GUI/Detect/coba" + coba + ".jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
                    coba++;
                }
                catch
                {
        ...
        private void button10_Click (object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog()==DialogResult.OK)
            {
               ...
               // bmp.Save(dialog.FileName, ImageFormatConverter.Jpeg);
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("Unauthorized Acces");
       ...
       /* private void backgroundWorker2_DoWork(object sender, DoWorkEventArgs e)
        {
            while (true)
            {
                serialPort1.Write(Convert.ToString("x" + coords1.Lat));
                Thread.Sleep(100);
                serialPort1.Write(Convert.ToString("y" + coords1.Lng));
                Thread.Sleep(100);
                serialPort1.Write(Convert.ToString("z" + coords1.Alt));
                Thread.Sleep(100);
                serialPort1.Write(Convert.ToString(MainV2.comPort.MAV.cs.DistToHome + "d"));
                Thread.Sleep(100);
        ...
        {
            float Lat = (float)coords1.Lat;
            float Lng = (float)coords1.Lng;
            float Alt = (float)coords1.Alt;
            float Dist = (float)MainV2.comPort.MAV.cs.DistToHome;

            string kirim = "x" + Lat + "y" + Lng + "z" + Alt + "d" + Dist +"\r\n";
            kirim = kirim.Replace(",", ".");

            char data = Convert.ToChar(serialPort1.ReadByte());
            if (data == 'g')
            {
                serialPort1.Write(kirim);
            }
            else
            {
                while (data != '\n')
                {
                    try
                    {
                        data = Convert.ToChar(serialPort1.ReadByte());
                        txt += data;
                    }
                    catch { Exception ex; }
                }
